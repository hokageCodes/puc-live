...
import { useHubAuth, useHubGuard, resolveHubSafeRedirect } from '../../components/hub/HubAuthContext';
...
function LoginInner() {
  ...
  const { signIn, status, isAuthenticated } = useHubAuth();

  // Already signed in → bounce to the requested page (or the hub dashboard).
  useHubGuard({ redirectIfFound: true, redirectIfFoundTarget: nextTarget });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // On success the provider flips to `authenticated`; useHubGuard redirects.
    } catch (err) {
  ...
  if (isAuthenticated) return <LoadingCard message="Redirecting…" />;
